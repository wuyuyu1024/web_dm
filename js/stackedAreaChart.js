 ...
 * @param  selectedIndex    -- da global 'variable' inside the class that keeps track of the index of the selected area
 */
...
constructor(parentElement, data) {
	...
	initVis(){
		...
		// vis.stackedData = vis.stackingData(vis.data)
		...
		// TO-DO (Activity IV): Add Tooltip placeholder  (top left corner)
		vis.tooltip = vis.svg.append("text")
			.attr("x", 10)
			.attr("y", 10)
			.attr("class", "slectionText")
			.text(" ");
	...
	stackingData(data){
		...
		vis.x.domain(d3.extent(data, d => d.Year));

		return vis.stack(data);
	}
	/*
 	* Data wrangling
 	*/
	wrangleData(data){
		let vis = this;
        // dataToShow = this.stackingData(data);
        vis.displayData = this.stackingData(data);
